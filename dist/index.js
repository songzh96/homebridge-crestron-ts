...
/*#__PURE__*/
function () {
  function BaseAccessory(log, accessoryConfig, platform) {
    ...
    var _this$platform$api$ha = this.platform.api.hap,
        Characteristic = _this$platform$api$ha.Characteristic,
        Service = _this$platform$api$ha.Service;
...
function getPowerState(callback) {
  ...
  api.once(`Response-${this.type}-${this.id}-Get-Power`, function (value) {
    platform.socket.pendingGetRequests.delete(`${_this.type}-${_this.id}-Power`);
    var powered = Boolean(value);
...
function setPowerState(powered, callback) {
  ...
  if ((this.type === 'LightDimmer' || this.type === 'Fan') && powered) {
    ...
    setTimeout(function () {
      if (!isLevelAlsoSet) {
        _this2.platform.socket.write(jsonMessage);

        platform.socket.pendingSetRequests.set(`${_this2.type}-${_this2.id}-Power`, jsonMessage);
        api.once(`Response-${_this2.type}-${_this2.id}-Set-Power`, function () {
          platform.socket.pendingSetRequests.delete(`${_this2.type}-${_this2.id}-Power`);
          callback();
        });
      } else {
        callback();
...
function getLightLevel(callback) {
  ...
  api.once(`Response-${this.type}-${this.id}-Get-Level`, function (value) {
    platform.socket.pendingGetRequests.delete(`${_this3.type}-${_this3.id}-Level`);
    var percentLevel = value * 100 / 65535;
...
function setLightLevel(percentLevel, callback) {
  ...
  api.once(`Response-${this.type}-${this.id}-Set-Level`, function () {
    platform.socket.pendingSetRequests.delete(`${_this4.type}-${_this4.id}-Level`);
    callback();
  });
}
function getFanSpeed(callback) {
  ...
  api.once(`Response-${this.type}-${this.id}-Get-Speed`, function (value) {
    platform.socket.pendingGetRequests.delete(`${_this5.type}-${_this5.id}-Speed`);
    var speed = value;
    callback(null, speed);
  });
}
function setFanSpeed(speed, callback) {
  ...
  api.once(`Response-${this.type}-${this.id}-Set-Speed`, function () {
    platform.socket.pendingSetRequests.delete(`${_this6.type}-${_this6.id}-Speed`);
    callback();
  });
}
...
var Fan =
/*#__PURE__*/
function (_BaseAccessory) {
  _inherits(Fan, _BaseAccessory);

  function Fan(log, accessoryConfig, platform) {
    var _this;

    _classCallCheck(this, Fan);

    _this = _possibleConstructorReturn(this, _getPrototypeOf(Fan).call(this, log, accessoryConfig, platform));

    _defineProperty(_assertThisInitialized(_this), "fanService", void 0);
  ...
  _createClass(Fan, [{
    ...
    value: function getServices() {
      ...
      var _api$hap = api.hap,
          Characteristic = _api$hap.Characteristic,
          Service = _api$hap.Service;
      var fanService = new Service.Fan();
      var powerState = fanService.getCharacteristic(Characteristic.On).on('get', getPowerState.bind(this)).on('set', setPowerState.bind(this));
      var fanSpeed = fanService.getCharacteristic(Characteristic.RotationSpeed).on('get', getFanSpeed.bind(this)).on('set', setFanSpeed.bind(this));
      this.fanService = fanService;
      api.on(`Event-${this.type}-${this.id}-Set-Power`, function (value) {
        powerState.updateValue(Boolean(value));
      });
      api.on(`Event-${this.type}-${this.id}-Set-Speed`, function (value) {
...
/*#__PURE__*/
function (_BaseAccessory) {
  _inherits(GenericSwitch, _BaseAccessory);

  function GenericSwitch(log, accessoryConfig, platform) {
    var _this;

    _classCallCheck(this, GenericSwitch);

    _this = _possibleConstructorReturn(this, _getPrototypeOf(GenericSwitch).call(this, log, accessoryConfig, platform));

    _defineProperty(_assertThisInitialized(_this), "lightBulbService", void 0);

    return _this;
  }

  _createClass(GenericSwitch, [{
    key: "getServices",
    value: function getServices() {
      var platform = this.platform;
      var api = platform.api;
      ...
      var powerState = switchService.getCharacteristic(Characteristic.On).on('get', getPowerState.bind(this)).on('set', setPowerState.bind(this));
...
/*#__PURE__*/
function (_BaseAccessory) {
  _inherits(LightDimmer, _BaseAccessory);

  function LightDimmer(log, accessoryConfig, platform) {
    var _this;

    _classCallCheck(this, LightDimmer);

    _this = _possibleConstructorReturn(this, _getPrototypeOf(LightDimmer).call(this, log, accessoryConfig, platform));
  ...
  _createClass(LightDimmer, [{
    ...
    value: function getServices() {
      ...
      var powerState = lightBulbService.getCharacteristic(Characteristic.On).on('get', getPowerState.bind(this)).on('set', setPowerState.bind(this));
      var lightLevel = lightBulbService.getCharacteristic(Characteristic.Brightness).on('get', getLightLevel.bind(this)).on('set', setLightLevel.bind(this));
      this.lightBulbService = lightBulbService;
      api.on(`Event-${this.type}-${this.id}-Set-Power`, function (value) {
        powerState.updateValue(Boolean(value));
      });
      api.on(`Event-${this.type}-${this.id}-Set-Level`, function (value) {
...
/*#__PURE__*/
function (_BaseAccessory) {
  _inherits(LightSwitch, _BaseAccessory);

  function LightSwitch(log, accessoryConfig, platform) {
    var _this;

    _classCallCheck(this, LightSwitch);

    _this = _possibleConstructorReturn(this, _getPrototypeOf(LightSwitch).call(this, log, accessoryConfig, platform));
...
function () {
  function Platform(log, config, api) {
    ...
    this.socket.on('close', function () {
      _this.log('Connection Lost. Attempting to reconnect in 10 seconds...');

      _this.socket.setTimeout(10000, function () {
        _this.socket.connect(port, host, function () {
          _this.log('Connection re-established with the Crestron Processor');
        });
      });
    }); // connect to the Crestron processor

    this.socket.connect(port, host, function () {
      _this.log(`Connected to the Crestron Processor @ ${host}`);
    }); // Retry pending `Get` and `Set` Requests every 2 seconds

    setInterval(function () {
      if (!_this.socket.pending) {
        if (_this.socket.pendingGetRequests.size > 0) {
          _this.socket.pendingGetRequests.forEach(function (value, key) {
            _this.log(`Retrying get request: ${key}`);

            _this.socket.write(value);
          });
        }

        if (_this.socket.pendingSetRequests.size > 0) {
          _this.socket.pendingSetRequests.forEach(function (value, key) {
            _this.log(`Retrying set request: ${key}`);
    ...
    this.socket.on('data', function (data) {
      var jsonMessages = data.toString().split('||').filter(function (jsonMessage) {
        return jsonMessage.length > 0;
      });
      jsonMessages.forEach(function (jsonMessage) {
        ...
        if (messageType === 'Event' && _this.socket.pendingSetRequests.has(`${deviceType}-${deviceId}-${property}`)) {
          _this.api.emit(`Response-${deviceType}-${deviceId}-${operation}-${property}`);

          return;
        }

        _this.api.emit(`${messageType}-${deviceType}-${deviceId}-${operation}-${property}`, value);
      });
    ...
    process.on('exit', function () {
      ...
      _this.log('Disconnected from the Crestron Processor');
  ...
  _createClass(Platform, [{
    ...
    value: function accessories(callback) {
      ...
      var devicesByType = undefined(devices, 'type');
      /*
      ...
      undefined(devicesByType, function (devices, type) {
        devices.forEach(function (device) {
          switch (type) {
            case 'LightSwitch':
              accessories.push(new LightSwitch(_this2.log, device, _this2));
              return;

            case 'LightDimmer':
              accessories.push(new LightDimmer(_this2.log, device, _this2));
              return;

            case 'GenericSwitch':
              accessories.push(new GenericSwitch(_this2.log, device, _this2));
              return;

            case 'Fan':
              accessories.push(new Fan(_this2.log, device, _this2));
              return;
          }
        });
      });
